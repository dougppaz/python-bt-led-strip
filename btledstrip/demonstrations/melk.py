...
async def main(mac_address: str = None) -> None:
    ...
    async with BTLedStrip(controller, mac_address) as led_strip:
        while True:
            ...
            await led_strip.exec_white(temperature=0)
            await led_strip.exec_white_brightness(percentage=100)
            await led_strip.exec_turn_on()
            time.sleep(1)
            await led_strip.exec_white(temperature=100)
            time.sleep(1)
            await led_strip.exec_white_brightness(percentage=0)
